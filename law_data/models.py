...
class Law(Base):
    ...
    chapters = relationship(
        "LawChapter", back_populates="law", cascade="all, delete-orphan"
    )
...
class LawChapter(Base):
    ...
    parts = relationship(
        "LawPart", back_populates="chapter", cascade="all, delete-orphan"
    )
...
class LawPart(Base):
    ...
    paragraphs = relationship(
        "LawParagraph", back_populates="part", cascade="all, delete-orphan"
    )
...
class LawParagraph(Base):
    ...
    )  # Номер пункта с подпунктом, если имеется
