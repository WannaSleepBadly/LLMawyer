...
class LawRepository:
    ...
    def __init__(self):
        self.session = get_db_manager().get_session()

    def create_law(self, law_name: str, law_code: str, source_url: str) -> Law:
        """Создание нового закона"""
        law = Law(name=law_name, code=law_code, source_url=source_url)
        self.session.add(law)
        self.session.flush()  # Получаем ID без коммита
        return law

    def create_chapter(
        self, law_id: int, chapter_number: int, title: str, source_url: str
    ) -> LawChapter:
        """Создание главы закона"""
        chapter = LawChapter(
            law_id=law_id, number=chapter_number, title=title, source_url=source_url
        )
    ...
    def create_part(
        self, chapter_id: int, part_number: str, title: str, source_url: str
    ) -> LawPart:
    ...
    def create_paragraph(
        self, part_id: int, paragraph_number: str, content: str
    ) -> LawParagraph:
        """Создание пункта закона"""
        paragraph = LawParagraph(
            part_id=part_id, number=paragraph_number, content=content
        )
    ...
    def get_law_by_code(self, law_code: str) -> Optional[Law]:
        ...
        return self.session.query(Law).filter(Law.code == law_code).first()

    def get_chapters_by_law(self, law_id: int) -> List[LawChapter]:
        """Получение всех глав закона"""
        return (
            self.session.query(LawChapter)
            .filter(LawChapter.law_id == law_id)
            .order_by(LawChapter.number)
            .all()
        )
    ...
    def get_parts_by_chapter(self, chapter_id: int) -> List[LawPart]:
        ...
        return (
            self.session.query(LawPart)
            .filter(LawPart.chapter_id == chapter_id)
            .order_by(LawPart.part_number)
            .all()
        )
    ...
    def get_paragraphs_by_part(self, part_id: int) -> List[LawParagraph]:
        ...
        return (
            self.session.query(LawParagraph)
            .filter(LawParagraph.part_id == part_id)
            .order_by(LawParagraph.paragraph_number)
            .all()
        )

    def search_in_content(
        self, search_term: str, law_code: str = None
    ) -> list[Type[LawParagraph]]:
