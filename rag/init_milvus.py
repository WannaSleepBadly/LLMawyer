...
def main():
    ...
    if not embedding_model:
        ...
        logger.info(
            "Добавьте в .env файл: EMBEDDING_MODEL=sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
        )
        return False
    ...
    try:
        ...
        if results:
            logger.info(
                f"✅ Тестовый поиск успешен. Найдено {len(results)} результатов:"
            )
            for i, result in enumerate(results[:3], 1):
                logger.info(
                    f"  {i}. {result['content'][:100]}... (score: {result['similarity_score']:.3f})"
                )
                logger.info(result)
        else:
