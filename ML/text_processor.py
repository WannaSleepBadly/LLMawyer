...
class TextProcessor:
    def __init__(self, rag_service: RAGSearchService, ollama_client: OllamaClient):
    ...
    def generate_response(self, text: str) -> str:
        ...
        try:
            ...
            retrieved_fragments = self.rag_service.search_similar_paragraphs(
                text, top_k=5
            )

            paragraphs = [fragment["content"] for fragment in retrieved_fragments]
            llm_answer = self.ollama_client.get_answer(text, paragraphs)
    ...
    @staticmethod
    def format_response(
        llm_answer: str, retrieved_fragments: list[dict[str, any]]
    ) -> str:
        """
        ...
        for i, frag in enumerate(retrieved_fragments, 1):
            ref = f"""{i}. <a href="{frag['law_url']}"> {frag['law_name']}</a> <a href="{frag['chapter_url']}"> {frag['chapter_title']}</a> <a href="{frag['part_url']}"> {frag['part_title']}</a> Пункт {frag['paragraph_number']}."""
